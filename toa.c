...
#define TOA_IP4_TAB_BITS    12
#define TOA_IP4_TAB_SIZE    (1 << TOA_IP4_TAB_BITS)
#define TOA_IP4_TAB_MASK    (TOA_IP4_TAB_SIZE - 1)

struct toa_ip4_entry {
    struct toa_ip4_data toa_data;
    struct toa_ip4_data vip_data;
    struct sock *sk;

    struct list_head list;
};

struct toa_ip4_list_head {
    struct list_head toa_ip4_head;
    spinlock_t lock;
} __attribute__((__aligned__(SMP_CACHE_BYTES)));

static struct toa_ip4_list_head
    __toa_ip4_list_tab[TOA_IP4_TAB_SIZE] __cacheline_aligned;

/* per-cpu lock for toa of ipv4  */
struct toa_ip4_sk_lock {
    /* lock for sk of ip6 toa */
    spinlock_t __percpu *lock;
};

static struct toa_ip4_sk_lock toa_ip4_sk_lock;
...
init_toa_ip4(void)
{
    ...
    toa_ip4_sk_lock.lock = alloc_percpu(spinlock_t);
    if (toa_ip4_sk_lock.lock == NULL) {
        TOA_INFO("fail to alloc per cpu ip4's destruct lock\n");
        return -ENOMEM;
    }
    ...
    for_each_possible_cpu(i) {
        ...
        lock = per_cpu_ptr(toa_ip4_sk_lock.lock, i);
        spin_lock_init(lock);
    }

    for (i = 0; i < TOA_IP4_TAB_SIZE; ++i) {
        INIT_LIST_HEAD(&__toa_ip4_list_tab[i].toa_ip4_head);
        spin_lock_init(&__toa_ip4_list_tab[i].lock);
    }
...
static inline unsigned int toa_ip4_hash_function(const struct sock *sk)
{
    ...
    u32 hash = jhash_3words(src_ip, dst_ip, (u32)(src_port << 16 | dst_port), 0);
    return hash % TOA_IP4_TAB_SIZE;
}

static inline __u32 ipv4_addr_jhash(__be32 addr, const u32 initval)
{
    return jhash_1word(addr, initval);
}

void toa_ip4_hash(struct data_entry *entry)
{
    struct toa_ip4_data *ptr_toa_data = &entry->cip_data.ip4;
    __u32 hash_key =
        ipv4_addr_jhash(ptr_toa_data->ip, ptr_toa_data->port) & TOA_IP4_TAB_MASK;

    spin_lock_bh(&__toa_ip4_list_tab[hash_key].lock);

    list_add(&entry->list, &__toa_ip4_list_tab[hash_key].toa_ip4_head);

    spin_unlock_bh(&__toa_ip4_list_tab[hash_key].lock);
}

void toa_ip4_unhash(struct data_entry *entry)
{
    struct toa_ip4_data *ptr_toa_data = &entry->cip_data.ip4;
    __u32 hash_key =
        ipv4_addr_jhash(ptr_toa_data->ip, ptr_toa_data->port) & TOA_IP4_TAB_MASK;
...
static void
lock_cpu_toa_ip4_sk(void)
{
    spinlock_t *lock = this_cpu_ptr(toa_ip4_sk_lock.lock);
    spin_lock_bh(lock);
}

static void
unlock_cpu_toa_ip4_sk(void)
{
    spinlock_t *lock = this_cpu_ptr(toa_ip4_sk_lock.lock);
    spin_unlock_bh(lock);
}
...
static void
toa_ip6_hash(struct data_entry *entry)
{
    struct toa_ip6_data *ptr_toa_data = &entry->cip_data.ip6;
    __u32 hash_key =
        __ipv6_addr_jhash(&ptr_toa_data->in6_addr, ptr_toa_data->port) & TOA_IP6_TAB_MASK;

    spin_lock_bh(&__toa_ip6_list_tab[hash_key].lock);

    list_add(&entry->list, &__toa_ip6_list_tab[hash_key].toa_ip6_head);

    spin_unlock_bh(&__toa_ip6_list_tab[hash_key].lock);
}

static void
toa_ip6_unhash(struct data_entry *entry)
{
    struct toa_ip6_data *ptr_toa_data = &entry->cip_data.ip6;
    __u32 hash_key =
        __ipv6_addr_jhash(&ptr_toa_data->in6_addr, ptr_toa_data->port) & TOA_IP6_TAB_MASK;
 ...
 */
static void get_toa_data(int af, struct sk_buff *skb, int *nat64, int *vip64, struct data_entry *entry)
{
    struct tcphdr *th;
    int length;
    unsigned char *ptr;
            ...
            default:
                ...
                if (TCPOPT_TOA == opcode &&
                    ...
                    TOA_DBG("af = %d, find toa data: ip = "
                        ...
                        ntohs(tdata.port));
                    if (af == AF_INET) {
                        memcpy(&entry->cip_data.ip4, &tdata,
                            sizeof(tdata));
                    }
#ifdef TOA_IPV6_ENABLE
                    else if (af == AF_INET6) {
                        struct toa_ip6_data *ptr_toa_ip6;
                        ptr_toa_ip6 = &entry->cip_data.ip6;
                ...
                if (TCPOPT_TOA == opcode &&
                    ...
                    if (af == AF_INET6)
                        ...
                        *nat64 = 1;
                        entry->protocol = AF_INET6;
                    }

                }

                if (TCPOPT_TOA_VIP == opcode &&
                    TCPOLEN_IP4_TOA == opsize) {
                    entry->protocol = af;
                    memcpy(&tdata, ptr - 2, sizeof(tdata));
                    if (af == AF_INET) {
                        // vip 4 server 4
                        memcpy(&entry->vip_data.ip4, &tdata, sizeof(tdata));
                    }
                    else if (af == AF_INET6) {
                        // vip 4 server 6
                        ptr_toa_ip6 = &entry->cip_data.ip6;
                ...
                if (TCPOPT_TOA_VIP == opcode &&
                    TCPOLEN_IP6_TOA == opsize) {
                    entry->protocol = af;
                    ptr_toa_ip6 = &entry->vip_data.ip6;
                    // 结构布局一样,可以直接copy,否则需要逐一赋值
                    // opcode opsize port in6_addr 20
                    memcpy(ptr_toa_ip6, ptr - 2, TCPOLEN_IP6_TOA);
                    if (af == AF_INET6)
                        *vip64 = 0;
                    else {
                        *vip64 = 1;
...
#endif
{
    ...
    if (NULL != sk->sk_user_data) {
        ...
        if (retval >= 0 && AF_INET == entry->protocol && peer) {
            if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready && !sock_flag(sk, SOCK_NAT64)) {
                memcpy(&tdata, &entry->cip_data.ip4, sizeof(tdata));
...
#endif
{
    ...
    TOA_DBG("inet6_getname_toa called, sk->sk_user_data is %p  pointer address:%p\n",
        sk->sk_user_data, sk);
    ...
    if (NULL != sk->sk_user_data) {
        struct data_entry *entry;
        entry = sk->sk_user_data;
        if (retval >= 0 &&  AF_INET6 != entry->protocol && peer) {
            if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready) {
                struct toa_ip6_data* ptr_ip6_data = &entry->cip_data.ip6;
                if (sk == entry->sk &&
...
#endif
{
    ...
    if (NULL != newsock && NULL == newsock->sk_user_data) {
        entry= kzalloc(sizeof(struct data_entry),GFP_ATOMIC);
        memset(entry, 0, sizeof(struct data_entry));
        INIT_LIST_HEAD(&entry->list);
        if (!entry) {
            return newsock;
        }
        get_toa_data(AF_INET, skb, &nat64, &vip64, entry);
        if (entry->protocol != AF_INET && entry->protocol != AF_INET6) {
            /* 说明没有检测到 vip 和 cip */
            kfree(entry);
            entry = NULL;
            return newsock;
        ...
        newsock->sk_user_data = entry;
        entry->sk = newsock;

        if (entry->protocol == AF_INET) {

            if (entry->cip_data.ip4.opcode == TCPOPT_TOA || entry->vip_data.ip4.opcode == TCPOPT_TOA_VIP) {
                toa_ip4_hash(entry);
            }

            newsock->sk_destruct = tcp_v4_sk_destruct_toa;
        }


        if (entry->protocol == AF_INET6)
        {
            if (entry->cip_data.ip6.opcode == TCPOPT_TOA || entry->vip_data.ip6.opcode == TCPOPT_TOA_VIP) {
                toa_ip6_hash(entry);
            }
        ...
        if (vip64)
            sock_set_flag(newsock, SOCK_VIP_OPT);
...
#endif
{
    ...
    if (NULL != newsock && NULL == newsock->sk_user_data) {
        ...
        get_toa_data(AF_INET6, skb, &nat64, &vip64, entry);
        if (entry->protocol != AF_INET6) {
            /* 说明没有检测到 vip 和 cip */
            kfree(entry);
            entry = NULL;
        ...
        if (entry->vip_data.ip6.opcode == TCPOPT_TOA_VIP)
            sock_reset_flag(newsock, SOCK_VIP_OPT);

        TOA_DBG("tcp_v6_syn_recv_sock_toa: set "
            "sk->sk_user_data to %p struct sock POINTER ADDRESS:%p :\n",
            newsock->sk_user_data, newsock);
    }
    return newsock;
}
#endif
...
unhook_toa_functions(void)
{
    ...
    TOA_INFO("CPU [%u] unhooked inet_getname  <%p>  ----> <%p>\n", smp_processor_id(),inet_getname_toa,inet_getname);
...
static int
inet_getvipinfo_toa(struct sock *sk, int optval, void __user *user, int *len)
{
    struct inet_sock *inet = NULL;
    struct data_entry *entry;
    struct toa_ip4_data tdata;
    struct toa_vip_info vip_info;
    int ret;
    if (optval != TOA_VIP_SO_GET_LOOKUP || !sk) {
        TOA_INFO("%s: bad cmd\n", __func__);
        return -EINVAL;
    }

    if (*len < sizeof(struct toa_vip_info) ||
        NULL == user) {
        TOA_INFO("%s: bad param len\n", __func__);
        return -EINVAL;
    }
    ...
    if (!entry)
        goto out;
    if (entry->protocol == AF_INET) {
        memcpy(&tdata ,&entry->vip_data.ip4, sizeof(tdata));
      if (tdata.opcode == TCPOPT_TOA_VIP && tdata.opsize == TCPOLEN_IP4_TOA) {
          vip_info.port = tdata.port;
          vip_info.family = AF_INET;
          vip_info.addr.ipv4.s_addr = tdata.ip;
          if (copy_to_user(user, &vip_info,sizeof(vip_info)) != 0) {
              ret = -EFAULT;
              goto out;
          }
          *len = sizeof(vip_info);
    ...
    } else if (entry->protocol == AF_INET6) {
        if (sk_data_ready_addr == (unsigned long) sk->sk_data_ready) {
            struct toa_ip6_data *ptr_ip6_data = &entry->vip_data.ip6;
            if (TCPOPT_TOA_VIP == ptr_ip6_data->opcode &&
                   TCPOLEN_IP6_TOA == ptr_ip6_data->opsize) {
                vip_info.port = ptr_ip6_data->port;
                vip_info.family = AF_INET6;
                vip_info.addr.ipv6 = ptr_ip6_data->in6_addr;
                if (copy_to_user(user, &vip_info,
                                 sizeof(vip_info)) != 0) {
                    ret = -EFAULT;
                    goto out;
                }
...
/* Toa vip socket options.  */
static struct nf_sockopt_ops toa_vip_sockopts = {
        .pf    = PF_INET,
        .owner    = THIS_MODULE,
        /* Nothing to do in set */
        /* get */
        .get_optmin = TOA_VIP_BASE_CTL,
        .get_optmax = TOA_VIP_SO_GET_MAX + 1,
        .get        = inet_getvipinfo_toa,
};
...
toa_init(void)
{
    ...
    if (0 != nf_register_sockopt(&toa_sockopts)) {
        TOA_INFO("fail to register toa_sockopts sockopt\n");
        goto err;
    }
#endif

    if (0 != nf_register_sockopt(&toa_vip_sockopts)) {
        TOA_INFO("fail to register toa_vip_sockopts sockopt\n");
        goto err;
    }
    ...
    TOA_INFO("toa loaded\n");
    TOA_INFO("This module is based on the DPVS VIP and CIP functionality. Supported features include CIP, VIP, and more. Version: %s\n", TOA_VERSION);
    return 0;
